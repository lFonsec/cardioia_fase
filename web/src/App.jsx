  ...
  async function analisar(payload = dados) {
    try {
      const resp = await fetch(`${API_URL}/api/sinais`, {
        ...
        body: JSON.stringify({ patient_id: 'paciente-web', origem: 'web', ...payload })
      });
      const json = await resp.json();
      setResultado(json);
      carregarLeituras();
    } catch (e) {
      setResultado({ risco: 'Offline', probabilidade: 0, recomendacao: 'Backend indisponível. Execute uvicorn app.main:app --reload na pasta backend.' });
    }
  }
